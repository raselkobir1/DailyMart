...
export const appConfig: ApplicationConfig = {
  providers: [
    ...
    // Loads the current user's permitted-menu list and shop name before the app renders any route, so the
    // authGuard/canView guards and the sidebar have real data on a hard refresh, not just after an in-app
    // login (see ShopBranding's doc comment for why the shop name needs its own load alongside Perms).
    ...
    provideAppInitializer(() => {
      ...
      const shopBranding = inject(ShopBranding);
      if (!auth.isAuthenticated()) {
        return Promise.resolve();
      }
      return firstValueFrom(forkJoin([perms.load(), shopBranding.load()]));
    })
  ]
};
